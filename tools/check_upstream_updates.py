...
def render_markdown(
    ...
    lines.extend(
        [
            "## Commits",
            "",
            f"{len(commits)} unreviewed upstream commit(s).",
            "",
            "| Commit | Date | Subject |",
            "| --- | --- | --- |",
        ]
    ...
    lines.extend(
        [
            "",
            f"Review each commit, record the verdict in `{decision_log}`, and",
            "advance `reviewed_through` in `tools/upstream_baseline.json`.",
            "",
        ]
    )
    return with_tickets(lines)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--strict",
        action="store_true",
        help="exit with non-zero when unreviewed commits or triage items are found",
    )
    parser.add_argument(
        "--output",
        type=Path,
...
def main() -> int:
    ...
    try:
        ...
        report = render_markdown(baseline, commits, prs=prs, issues=issues)
        has_work = bool(commits) or bool(prs) or bool(issues)
    except UpstreamCheckError as exc:
        try:
            baseline = load_baseline()
        except UpstreamCheckError:
            baseline = {
                "repo": "unknown",
                "branch": "unknown",
        ...
        report = render_markdown(baseline, [], error=str(exc))
        has_work = True
