...
本 repo fork 自 [`doggy8088/codex-reset-checker`](https://github.com/doggy8088/codex-reset-checker)，
沿用 MIT License 與完整 Git 歷史。

## 為什麼維護 fork

- 保留原作者（Will 保哥）持續開發的 Codex/ChatGPT 額度查詢、重置到期時程與手動重置 CLI 工具。
...
| `tools/dev_check.ps1` | Windows 本機一鍵 gate（整合 Python 契約與 Node 產品測試） |
| `tools/bootstrap_dev.ps1` | Windows 本機一鍵初始化與驗收 |
| `tools/test_product.ps1` | Windows 原生產品測試執行腳本 |
| `requirements-dev.txt` | 維護測試工具依賴清單（pytest、ruff） |
| `.github/workflows/ci.yml` | 強化安全：針對 Windows 原生 Node.js 矩陣與 gate 驗收，防止 fork 觸發上游 npm publish |
...
# git remote add upstream https://github.com/doggy8088/codex-reset-checker.git
pwsh -NoProfile -File tools\bootstrap_dev.ps1
```

要實際使用 CLI 與查看額度，見 [`docs/DEVELOPMENT.md`](docs/DEVELOPMENT.md)。
