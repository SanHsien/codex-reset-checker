...
function createFakeHttps(responses) {
  ...
  https.request = (options, callback) => {
    ...
    request.end = () => {
      ...
      if (Array.isArray(responseConfig.chunks)) {
        for (const chunk of responseConfig.chunks) {
          response.emit('data', chunk);
        }
      } else if (responseConfig.body !== undefined && responseConfig.body !== '') {
...
async function testRequestJsonPreservesMultiByteUtf8AcrossChunkBoundaries() {
  const text = JSON.stringify({ message: '繁體中文測試🚀與額度查詢' });
  const fullBuffer = Buffer.from(text, 'utf8');
  const prefixLength = Buffer.from('{"message":"', 'utf8').length;
  // 切在多位元組字元「繁」(0xE7, 0xB9, 0x81) 的第 1 個 byte 後
  const cutIndex = prefixLength + 1;
  const chunk1 = fullBuffer.subarray(0, cutIndex);
  const chunk2 = fullBuffer.subarray(cutIndex);
  ...
  await withFakeHttps({
    ...
    const result = await checker.requestJson(
      'https://chatgpt.com/test-utf8',
      'test-token',
      'test-account'
    );
    assert.strictEqual(result.message, '繁體中文測試🚀與額度查詢');
  });
}

const tests = [
  ['API 請求支援跨 chunk 多位元組 UTF-8 字元完整解析', testRequestJsonPreservesMultiByteUtf8AcrossChunkBoundaries],
